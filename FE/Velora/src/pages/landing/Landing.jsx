...
import { LANDING_DEFAULTS, LANDING_ICONS } from '../../constants/landingConstants'
        ...
        <section style={styles.section}>
          ...
          <div style={styles.featureGrid}>
            {(content.features || []).map((feature, index) => {
              const Icon = LANDING_ICONS[feature.icon] || IconStar
              return <div key={index} style={{ ...styles.featureCard, ...(editor ? styles.editableCard : {}) }}>
                <div style={styles.featureIcon}><Icon size={18} /></div>
                {editor && <select value={feature.icon} onChange={event => editor.setListItem('features', index, 'icon', event.target.value)} style={styles.iconSelect}>{Object.keys(LANDING_ICONS).map(icon => <option key={icon}>{icon}</option>)}</select>}
