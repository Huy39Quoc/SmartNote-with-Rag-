...
import flashcardApi from '../../lib/api/flashcardApi' // Import API Flashcard
...
export default function GhiChu() {
    ...
    // --- STATE QUẢN LÝ TIẾN TRÌNH GENERATE AI FLASHCARD ---
    ...
    const handleTaoFlashcard = async () => {
        ...
        try {
            ...
            console.error(error)
            toast.error(error.response?.data?.message || 'Không thể kết nối với mô hình AI lúc này.', { id: loadToast })
        } finally {
    ...
    const taoTagMoi = async () => {
        ...
        if (!name) { toast.error('Vui lòng nhập tên tag'); return; }
        try {
            const {data} = await tagApi.taoMoi({ name, color: mauTagMoi })
            ...
            setGhiChuHienTai(p => {
                ...
                return { ...p, tags: daCo ? currentTags : [...currentTags, tagMoi] }
            })
            toast.success('Đã tạo tag mới')
        } catch (error) { toast.error('Không thể tạo tag') }
    }
    ...
    const toggleTagChoGhiChu = (tag) => {
        ...
        setGhiChuHienTai(p => {
            ...
            return { ...p, tags: daCo ? currentTags.filter(t => t.id !== tag.id) : [...currentTags, tag] }
        })
    }
    ...
    useEffect(() => {
        scheduleApi.layTatCa()
            .then(r => {
                ...
                const ids = new Set(schedules.filter(s => s.noteId).map(s => s.noteId))
                ...
                <div style={styles.listHeader}>
                    <div style={styles.searchRow}>
                        <div style={styles.searchWrap}>
                            ...
                            <input placeholder="Tìm kiếm..." style={{ paddingLeft: 26, fontSize: 12, height: 30 }} value={timKiem} onChange={e => setTimKiem(e.target.value)} />
                        </div>
                    ...
                    <div style={styles.tagFilter}>
                        ...
                        {tags.map(t => (
                            <button key={t.id} style={{ fontSize: 10, padding: '2px 8px', background: locTag === t.id ? 'var(--bg-ai)' : 'transparent' }} className={locTag === t.id ? 'btn-ai' : 'btn-ghost'} onClick={() => setLocTag(locTag === t.id ? null : t.id)}>{t.name}</button>
                        ))}
                    </div>
                </div>
                <div style={styles.listBody}>
                    {dangTai ? <div style={{ display: 'flex', justifyContent: 'center', padding: 20 }}><Spinner /></div> : danhSach.length === 0 ? <EmptyState icon={IconSearch} title="Không tìm thấy ghi chú" desc="Thử tạo ghi chú mới" /> : danhSach.map(n => (<NoteCard key={n.id} note={n} active={ghiChuHienTai?.id === n.id} onClick={() => chonGhiChu(n.id)} />))}
                </div>
            </div>
            ...
            <div style={styles.editor}>
                {!ghiChuHienTai ? (
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <EmptyState icon={IconPlus} title="Chọn hoặc tạo ghi chú" action={<button className="btn-primary" onClick={taoMoi}><IconPlus size={13} />Ghi chú mới</button>} />
                    </div>
                ) : (
                    <>
                        {/* Toolbar chỉnh sửa */}
                        <div style={styles.toolbar}>
                            <input value={ghiChuHienTai.title} onChange={e => setGhiChuHienTai(p => ({ ...p, title: e.target.value }))} placeholder="Tiêu đề ghi chú..." style={styles.tieuDe} />
                            <div style={styles.toolbarRight}>
                                ...
                                <button className="btn-ghost" onClick={danhDau} title={ghiChuHienTai.isBookmarked ? 'Bỏ đánh dấu' : 'Đánh dấu'}>
                                    {ghiChuHienTai.isBookmarked ? <IconBookmarkFilled size={14} style={{ color: 'var(--accent-amber)' }} /> : <IconBookmark size={14} />}
                                </button>
                                ...
                                <button className={hienTag ? 'btn-ai' : 'btn-ghost'} onClick={() => setHienTag(p => !p)} title="Gắn tag">
                                    <IconTag size={14}/><span style={{fontSize: 11}}>Tag</span>
                                </button>
                                <button className="btn-danger btn-ghost" onClick={xoa} title="Xoá"><IconTrash size={13} /></button>
                                <button className="btn-primary" onClick={luu} style={{ padding: '4px 12px', fontSize: 12 }}><IconCheck size={12} /> Lưu</button>
                            </div>
                        </div>

                        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                            <textarea value={ghiChuHienTai.content || ''} onChange={e => setGhiChuHienTai(p => ({ ...p, content: e.target.value }))} placeholder="Bắt đầu ghi chú... (hỗ trợ Markdown)" style={styles.textarea} />

                            {hienLichGoiY && (
                                <div style={styles.schedulePrompt}>
                                    <div style={{ fontSize: 12, marginBottom: 8 }}><strong>AI phát hiện thông tin thời gian.</strong> Bạn có muốn trích xuất lịch / công việc?</div>
                                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                        <button className="btn-primary" onClick={trichXuatLich} disabled={dangTrichLich} style={{ flex: 1, justifyContent: 'center', fontSize: 12 }}>{dangTrichLich ? 'Đang xử lý...' : 'Có, tạo lịch'}</button>
                                        <button className="btn-ghost" onClick={() => setHienLichGoiY(false)} style={{ flex: 1, justifyContent: 'center', fontSize: 12 }}>Không, để sau</button>
                                    </div>
                                </div>
                            )}
                            ...
                            {hienTag && (
                                <div style={styles.tagPanel}>
                                    <div style={styles.tagPanelHeader}>
                                        <span style={{display: 'flex', alignItems: 'center', gap: 5}}><IconTag size={13} style={{color: 'var(--accent-blue-dim)'}}/><span style={{fontSize: 12, fontWeight: 500}}>Tag ghi chú</span></span>
                                        <button className="btn-ghost" onClick={() => setHienTag(false)} style={{padding: 3}}><IconX size={13}/></button>
                                    </div>
                                    <div style={styles.tagPanelBody}>
                                        <div style={styles.tagCreateBox}>
                                            <input value={tenTagMoi} onChange={e => setTenTagMoi(e.target.value)} placeholder="Tên tag mới..." style={{fontSize: 12, height: 30}} />
                                            <div style={{display: 'flex', gap: 6}}>
                                                <input type="color" value={mauTagMoi} onChange={e => setMauTagMoi(e.target.value)} style={styles.colorInput} />
                                                <button className="btn-primary" onClick={taoTagMoi} style={{flex: 1, justifyContent: 'center', fontSize: 12}}><IconPlus size={12}/> Tạo tag</button>
                                            </div>
                                        </div>
                                        <div style={styles.tagListBox}>
                                            <div style={styles.tagSectionTitle}>Tag hiện có</div>
                                            {tags.length === 0 ? <div style={{fontSize: 12, color: 'var(--text-muted)'}}>Chưa có tag nào</div> : tags.map(tag => {
                                                const selected = ghiChuHienTai.tags?.some(t => t.id === tag.id)
                                                return (
                                                    <button key={tag.id} className={selected ? 'btn-ai' : 'btn-ghost'} onClick={() => toggleTagChoGhiChu(tag)} style={{ ...styles.tagSelectItem, borderColor: selected ? tag.color : 'var(--border)' }}>
                                                        <span style={{ width: 8, height: 8, borderRadius: '50%', background: tag.color || '#3B82F6', flexShrink: 0 }} />
                                                        <span>{tag.name}</span>
                                        ...
                                        <button className="btn-primary" onClick={luu} style={{width: '100%', justifyContent: 'center', fontSize: 12}}>Lưu tag cho ghi chú</button>
                                    </div>
                                </div>
                            )}

                            {hienAi && <AiPanel noteId={ghiChuHienTai.id} content={ghiChuHienTai.content} title={ghiChuHienTai.title} onApply={apDungAi} onDong={() => setHienAi(false)} />}
                        </div>
                    </>
                )}
            </div>
...
const styles = {
    ...
    schedulePrompt: { width: 320, flexShrink: 0, background: 'var(--bg-elevated)', border: '.5px solid var(--border)', borderRadius: 10, padding: 14, marginLeft: 12, display: 'flex', flexDirection: 'column', justifyBetween: 'space-between' },
    tagPanel: { width: 280, background: 'var(--bg-surface)', borderLeft: '.5px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0 },
    tagPanelHeader: { display: 'flex', justifyBetween: 'space-between', alignItems: 'center', padding: '10px 12px', borderBottom: '.5px solid var(--border)' },
    tagPanelBody: { padding: 12, display: 'flex', flexDirection: 'column', gap: 12, overflowY: 'auto', flex: 1 },
    tagCreateBox: { display: 'flex', flexDirection: 'column', gap: 8, padding: 10, background: 'var(--bg-elevated)', border: '.5px solid var(--border)', borderRadius: 6 },
    colorInput: { width: 36, height: 30, padding: 0, border: '.5px solid var(--border)', borderRadius: 6, background: 'transparent' },
    tagListBox: { display: 'flex', flexDirection: 'column', gap: 6 },
    tagSectionTitle: { fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 },
    tagSelectItem: { display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-start', fontSize: 12, padding: '6px 8px', border: '.5px solid var(--border)', borderRadius: 6 }
}