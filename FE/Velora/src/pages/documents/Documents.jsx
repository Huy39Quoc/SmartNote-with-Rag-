    ...
    const id = setInterval(async () => {
        for (const d of xuLy) {
            try {
                ...
                setDanhSach(p => p.map(x => x.id === d.id ? nextDoc : x))
...
const chiaSeTaiLieu = async () => {
    ...
    if (!chon?.id) {
        toast.error('Chưa chọn tài liệu để chia sẻ')
        return
    }
    ...
    try {
        ...
        setDocumentShareEmail('')
        setDocumentSharePermission('VIEW')
        await taiDanhSachShareTaiLieu()
    } catch (error) {
        toast.error(error.response?.data?.message || 'Không thể chia sẻ tài liệu')
    } finally {
                    ...
                    <div style={styles.docHeader}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                                {TRANG_THAI[chon.status]?.nhan || chon.status} · {dungLuong(chon.fileSize)}
                        ...
                        {isDocumentReady(chon) && coTeamWork && !chon._shared && (
                            <button
                                ...
                                onClick={() => setHienShareTaiLieu(p => !p)}
                                style={{ gap: 4 }}
                            ...
                            <div style={styles.sharePanel}>
                                <div style={styles.sharePanelHeader}>
                                    <div style={styles.panelTitle}>
                                        <IconShare size={13} style={{ color: 'var(--accent-blue-dim)' }} />
