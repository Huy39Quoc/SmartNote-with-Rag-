...
public class NoteExportServiceImpl implements NoteExportService {
        ...
        try (PDDocument document = new PDDocument();
            ...
            for (String rawLine : content.split("\\R", -1)) {
                List<String> wrappedLines = rawLine.isBlank()
                        ? List.of("") : wrapLine(rawLine, font, fontSize, maxWidth);
    ...
    private List<String> wrapLine(String text, PDType0Font font,
    ...
    private List<String> splitLongWord(String word, PDType0Font font,
                                       float fontSize, float maxWidth) throws IOException {
    ...
    private String buildMetaText(Note note) {
        if (note.getUpdatedAt() == null) return "Export từ Velora SmartNote AI";
    ...
    private String buildFileName(Note note, String extension) {
        ...
        if (title.isBlank()) title = "ghi-chu";
