...
public class UserPackageServiceImpl implements UserPackageService {
    ...
    @Override
    @Transactional
    public Map<String, Object> checkoutPackage(UUID packageId, String type, String email, String clientIp) {
        ...
        // Xử lý gói FREE hoặc gói có giá trị bằng 0
    ...
    private void activatePaidPackage(PackageTransaction tx) {
        ...
        LocalDateTime baseExpiryDate = (user.getPackageExpiryDate() != null && user.getPackageExpiryDate().isAfter(now))
                ? user.getPackageExpiryDate()
                : now;

        BigDecimal yearlyPrice = pkg.getPriceYearly() == null ? null : BigDecimal.valueOf(pkg.getPriceYearly());
